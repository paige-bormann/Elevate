...
public interface WorkoutDao { //create methods and annotate, room will automatically generate code for us
    ...
    @Query("SELECT rowid, name, description, style, grade, tutorial, completed FROM workout_table WHERE rowid LIKE :mWorkoutId LIMIT 1")
    ...
    // * means all columns
    @Query("SELECT rowid, name, description, style, grade, tutorial, completed FROM workout_table ORDER BY rowid DESC") //get all of the workouts?
